...
(function() {
    ...
    Bodies.edge = function (x1, y1, x2, y2, lw, options) {
        _minLength = 0.000001;
        _lineWidth = 1;
        options = options || {};
        ...
        var pA = { x: x1, y: y1 };
        var pB = { x: x2, y: y2 };
        var delta = Vector.sub(pA, pB);

        // prevent singularity
        var len = Vector.magnitude(delta);
        if (len < _minLength) {
            len = _minLength;
        }
        ...
        var edge = {
            label: 'Edge Body',
            position: { x: x1, y: y1 },
            vertices: Vertices.fromPath('L '+ x1 + ' ' + y1 + ' L ' + x2 + ' ' + y2),
            pointA: pA,
            pointB: pB,
            length: len,
            isStatic: true,
        ...
        return Body.create(Common.extend({}, edge, options));
    };
